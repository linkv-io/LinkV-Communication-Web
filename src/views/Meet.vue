<template>
	<div class="meet">
		<Nav  v-bind:isShowContent="true" v-bind:roomId.sync="roomId" v-bind:userId.sync="userId" />
		<div class="content">
			...
			<div class="rignt-fixed" :class="
			{active: this.rightStreamList.length != 0 && isShowRightList}">
				<ul class="list">
		...
		<div class="right-button-fixed" :class="{'btn-active': this.rightStreamList.length != 0 && isShowRightList}" @click="foldFun">
			<i class="triangle" :class="{'triangle-active': this.rightStreamList.length != 0 && isShowRightList}"></i>
		</div>
		<ul class="tool">
			<li class="item share" :class="{active: isActiveShare}" @click="display" v-if="source == 1 && devidePlatform == 'desktop'"></li>
			<li class="item mute" :class="{active: isActiveMute}" @click="mute" v-if="source == 1 || (source == 2 && isShowJoin)"></li>
			<li class="item join" :class="{active: isShowJoin}" @click="join" v-if="source == 2"></li>
			<li class="item record" :class="{active: isActiveCamera}" @click="record" v-if="isShowRecord"></li>
...
export default {
	...
	computed: {
		isSettingShowVideoDevice() {
			const res = (this.source == 1 || (this.source == 2 && this.isShowJoin)) && this.scene == 'video';
	...
	async mounted() {
		...
		console.log('get init params:::', this.$route.params, this.resultSettingConfig, this.scene);
		this.pageInit();
	},
	methods: {
		...
		async pageInit() {
			...
			this.logLevel = this.getQueryString('logLevel') == void 0 ? 0 : this.getQueryString('logLevel');
			this.enableLogUpload = this.getQueryString('enableLogUpload') == void 0 ? true : this.getQueryString('enableLogUpload');
			...
			if (crystalBallSource == void 0 && routerParams.source == void 0) { 
				...
				let role = 1;
				this.source = routerParams.source ? routerParams.source : crystalBallSource; // 1为推流 2为拉流 3为水晶球展示
				...
				switch (parseInt(this.source)) {
					...
					case 3:
						...
						break;
					case 4:
						const direction	= this.getQueryString('direction') == void 0 ? 1 : this.getQueryString('direction'); // 1: 推流 2: 拉流
						this.roomId 	= this.getQueryString('roomId');
						...
						this.currentConfig.channelCount = this.getQueryString('channelCount');
						this.currentConfig.audioBitrate = this.getQueryString('audioBitrate');
				...
				if (streamListTemp.length != 0) {
					streamListTemp.forEach((value, key, arr) => {
		...
		async creatPublishStream() {
			this.previewResult = await this.startPreview({camera: {
					audio: true,
			...
			console.log('startPreview stream0000:::', this.previewResult, this.streamList);
		...
		handleCallbackFun() {
			...
			const playCallback = ({ code, streamId, state }) => {
				...
				const type = code;
				this.streamList.forEach(async(value, key, arr) => {
				...
				this.playStatsTimer[streamId] = setInterval(async () => {
					const audioResult = await this.octopusRTC.getRemoteAudioStats(streamId);
					const videoResult = await this.octopusRTC.getRemoteVideoStats(streamId);
			...
			// add by version2.0.0
			this.octopusRTC.on('publish-state-update', ({code, streamId, state}) => {
				...
				this.publishStatsTimer[streamId] = setInterval(async () => {
					const audioResult = await this.octopusRTC.getLocalAudioStats(streamId);
					const videoResult = await this.octopusRTC.getLocalVideoStats(streamId);
		...
		async getOctopusToken() {
			const response = await fetch(`${this.currentConfig.BXMUrl}/api/v1/get_octopus_token`, {
				body: JSON.stringify({
		...
		async getLiveRoomId() {
			try {
				const result = await fetch(`${this.currentConfig.BXMUrl}/api/v1/gen_room`, {
					body: JSON.stringify({
						app_id: this.currentConfig.appId
					}),
					method: 'POST'
		...
		async updateRoom(status) {
			...
			const result =  await fetch(`${this.currentConfig.BXMUrl}/api/v1/update_room`, {
				body: JSON.stringify({
					app_id: this.currentConfig.appId,
		...
		async display() {
			if (this.isActiveShare) {
				...
				const stream = await this.startPreview({camera: {
					audio: true,
					audioInput: this.currentConfig.audioDevices,
					video: this.scene == 'audio' ? false : true,
					videoInput: this.currentConfig.videoDevices,
				...
				this.streamList.forEach((item, key) => {
					if (item.userId == this.userId) {
						this.octopusRTC.replaceTrack(item.stream, stream.getVideoTracks()[0],this.userId)
					}
				})
			} else {
