<template>
  <div class="settings" v-if="isShow">
    ...
    <ul class="list">
      <li class="item" v-for="(item, index) in settingArrList" :key="index">
...
export default {
  props: [
    "isShowSet",
    "source",
    "isSettingShowVideoDeviceOptions",
    "deviceConfigObj",
  ],
  data() {
    return {
  ...
  watch: {
    scene(n) {
      // 场景切换时 每个选项需要重置状态
      if (
        !this.currentSettingConfig ||
        Object.keys(this.currentSettingConfig).length == 0
      ) {
        return;
      }
      this.switchState(n);
    },
    isSettingShowVideoDeviceOptions(n) {
      if (n) {
  ...
  methods: {
    ...
    async pageLoad() {
      ...
      console.log(
        "get setting config:::",
        this.storeSettingsConfig,
        Object.keys(this.storeSettingsConfig)
      );
    ...
    //   let currentSelect = this.settingArrList[index];
      if (!this.settingArrList[index].fold) {
        ...
        this.settingArrList.forEach((value, key) => {
          value.fold = true;
